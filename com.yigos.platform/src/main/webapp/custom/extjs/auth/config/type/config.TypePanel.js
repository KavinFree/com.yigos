Ext.define('config.TypePanel', {
	...
	initGrid : function(){
		var me = this;
		me.typeGrid = Ext.create('Ext.grid.Panel', {
			layout: 'fit',
			xtype: 'grid',
		    loadMask: true,
		    columnLines: true,
	        ...
	        dockedItems:[{
	        	...
	        	items:[{text:me.configParam.i18n.addButton, minWidth:80, id:'add-button',
	        		...
	        		handler:function(){
	        			var selModel = me.typeGrid.getSelectionModel();
		                var ids = [];
			...
			buttons: [
				{
	                ...
	                handler : function() {
	                	...
	                	var savePath = "";
	                	if(me.updateFlag){
	                		savePath = me.configParam.urlsPath.updateType;
	                	}else{
	                		savePath = me.configParam.urlsPath.saveType;
	                	}
	...
	loadFormData : function(ids){
		...
		Ext.Ajax.request({
			url : me.configParam.urlsPath.findType,
			params : {id : ids},
			method : 'POST',
			timeout: 60000,
	...
	createDeleteWindow : function(ids){
		...
		Ext.Msg.confirm("提示","请确定要执行删除操作吗?", function (btn){
			if(btn == 'yes'){
				Ext.Ajax.request({
					url : me.configParam.urlsPath.deleteType,
					params : {ids : ids},
					method : 'POST',
					timeout: 60000,
					callback: function (options, success, response) {
