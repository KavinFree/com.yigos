Ext.define('config.ParamPanel', {
	...
	initStore : function(){
		...
		me.store = Ext.create('Ext.data.Store',{
			...
			fields: [
		        ...
		        { name: 'sysConfigType.name', type: 'string' }
		    ]
		});
		me.store.on('beforeload', function (store, options) {
			var newParams = {
					'query.code': Ext.getCmp('code-search').getValue(),
					'query.name': Ext.getCmp('name-search').getValue()
				};
	...
	initGrid : function(){
		var me = this;
		me.paramGrid = Ext.create('Ext.grid.Panel', {
			layout: 'fit',
			xtype: 'grid',
		    loadMask: true,
		    columnLines: true,
	        ...
	        dockedItems:[{
	        	...
	        	items:[{text:me.configParam.i18n.addButton, minWidth:80, id:'add-button',
	        		...
	        		handler:function(){
	        			var selModel = me.paramGrid.getSelectionModel();
		                var ids = [];
		    ...
		    columns: [
		        ...
		        { text : me.configParam.i18n.categoryId, dataIndex: 'sysConfigType.name', flex: 15, menuDisabled:true, sortable: true},
		        {
    ...
    initCategoryIdStore : function(){
		var me = this;
		me.categoryIdStore = Ext.create('Ext.data.Store',{
			autoLoad: true,
		    pageSize: CommonsUtil.pageSizeForCombo,
		    remoteSort: true,
			proxy:{
				type: 'ajax',
				url: me.configParam.urlsPath.categoryList,
				reader: {
					type:'json',
					root: 'result',
	            	totalProperty: 'totalCount'
		...
		me.formPanel = Ext.create('Ext.form.Panel', {
        	...
        	items:[
        		{xtype : 'hiddenfield', name: 'id', id:'formId'},
				{
                    ...
                    items: [
                    	{
		        			fieldLabel : '编号', name: 'code', width:'25%', allowBlank: false
						},
						{
		        			fieldLabel : '名称', name: 'name', width:'25%', allowBlank: false
						},
						{
		        			fieldLabel : '分类', name: 'categoryId', width:'25%', allowBlank: false,
		        			xtype: 'combo',
		                    store: me.categoryIdStore,
		                    displayField: 'name',
		                    valueField: 'id',
		                    listConfig: {
		                        maxHeight: 150
						...
						{
							fieldLabel : '状态', width:'25%', allowBlank: false, xtype: 'radiogroup',
							items: [
			...
			buttons: [
				{
	                ...
	                handler : function() {
	                	...
	                	var savePath = "";
	                	if(me.updateFlag){
	                		savePath = me.configParam.urlsPath.updateParam;
	                	}else{
	                		savePath = me.configParam.urlsPath.saveParam;
	                	}
	...
	loadFormData : function(ids){
		...
		|| ids===null || ids.length===0){
			if(me.loadMask != undefined||me.loadMask != null){ me.loadMask.hide();}
			return ;
		}
		Ext.Ajax.request({
			url : me.configParam.urlsPath.findParam,
			params : {id : ids},
			method : 'POST',
			timeout: 60000,
	...
	createDeleteWindow : function(ids){
		...
		Ext.Msg.confirm("提示","请确定要执行删除操作吗?", function (btn){
			if(btn == 'yes'){
				Ext.Ajax.request({
					url : me.configParam.urlsPath.deleteParam,
					params : {ids : ids},
					method : 'POST',
					timeout: 60000,
					callback: function (options, success, response) {
