Ext.define('auth.MenuPanel', {
	...
	initGrid : function(){
		...
		me.menuGrid = Ext.create('Ext.tree.Panel', {
	        ...
	        columns: [
  			    ...
  			    { text: me.configParam.i18n.text, dataIndex: 'text', xtype: 'treecolumn', flex: 1 },//iconCls
  	        	{ text: me.configParam.i18n.link, dataIndex: 'link',flex: 1 },
        	...
        	items:[
        		{xtype : 'hiddenfield', name: 'id', id:'formId'},
				{
                    ...
                    items: [
						...
						{
		        			fieldLabel : '名称', name: 'text', flex: 1, allowBlank: false
						},
						{
				...
				},
				{
                    ...
                    items: [
                    	{
		        			fieldLabel : '上级目录', name: 'pid', value:'0',
							multiSelect: true,
					        store: me.store,
							displayField : 'text',
							valueField : 'id',
				...
				},
				{
                    xtype: "fieldcontainer", layout: "hbox", defaults:{xtype:'textfield'},
                    items: [
                    	{fieldLabel : '链接', name: 'link', value:'/', width: '98%'}
                    ]
                },
				{
			...
			buttons: [
				{
	                ...
	                handler : function() {
	                	...
	                	var savePath = "";
	                	if(me.updateFlag){
	                		savePath = me.configParam.urlsPath.updateMenu;
	                	}else{
	                		savePath = me.configParam.urlsPath.saveMenu;
	                	}
	...
	loadFormData : function(ids){
		...
		Ext.Ajax.request({
			url : me.configParam.urlsPath.findMenu,
			params : {id : ids},
			method : 'POST',
			timeout: 60000,
	...
	createDeleteWindow : function(ids){
		...
		Ext.Msg.confirm("提示","请确定要执行删除操作吗?", function (btn){
			if(btn == 'yes'){
				Ext.Ajax.request({
					url : me.configParam.urlsPath.deleteMenu,
					params : {ids : ids},
					method : 'POST',
					timeout: 60000,
					callback: function (options, success, response) {
